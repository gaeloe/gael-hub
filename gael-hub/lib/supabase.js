...
export function friendlyDbError(error) {
  if (error.code === "42703" || error.code === "PGRST204" || error.code === "PGRST205" || error.code === "42P01") {
    return "The database is missing newer columns or tables — run the latest supabase-migrate-*.sql files in the Supabase SQL editor, then retry.";
  }
